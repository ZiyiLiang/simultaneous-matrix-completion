...
# Compute extremeness-based weights for MNAR mechanism
# Weights depend on how extreme values are relative to the data distribution
mnar_weights = compute_extremeness_weights(M_true, rho=rho, normalize=True)

if verbose:
    print('MNAR parameter rho = {:.3f}'.format(rho))
    if rho == 1.0:
        print('Using uniform observation weights (MAR).\n')
    else:
        print('Using extremeness-based observation weights (MNAR).')
