...
idir <- "results/exp_mnar_linear/"
ifile.list <- list.files(idir)

# Output directory
fig.dir <- "C:/Users/liang/Documents/GitHub/conformal-matrix-completion/results/figures/exp_mnar_linear"
...
make_plot <- function(results, exp, val, rho_str, xmax=2000, sv=TRUE) {
  ...
  if (exp=="vary_k"){
    pp <- results %>%
      ...
      theme_bw()
    if (sv == TRUE){
      ggsave(sprintf("%s/exp_mnar_linear_%s_rho%s.pdf", fig.dir, exp, rho_str), pp, device=NULL, width=6.5, height=height)}
    else{print(pp)}
  }
  else{
    pp <- results %>%
