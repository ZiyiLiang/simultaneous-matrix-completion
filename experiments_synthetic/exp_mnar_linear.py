...
if True:
    ...
    r = int(sys.argv[3])
    mu = int(sys.argv[4])
    rho = float(sys.argv[5])  # MNAR parameter: rho=1 gives MCAR, rho>1 gives MNAR
    seed = int(sys.argv[6])
...
# Output file #
###############
outdir = "./results/exp_mnar_linear/"
os.makedirs(outdir, exist_ok=True)
outfile_name = str(n1) + "by" + str(n2) + "_r" + str(r) + "_mu" + str(mu) + "_rho" + str(rho) + "_seed" + str(seed)
...
# Compute extremeness-based weights for MNAR mechanism using linear formula
# w = 1 + (rho - 1) * |M - median(M)|
# When rho = 1, all weights equal 1 (MCAR)
# When rho > 1, weights increase linearly with extremeness
mnar_weights = compute_extremeness_weights_linear(M_true, rho=rho, normalize=True)

if verbose:
    print('MNAR parameter rho = {:.3f}'.format(rho))
    if rho == 1.0:
        print('Using uniform observation weights (MCAR).\n')
    else:
        print('Using linear extremeness-based observation weights (MNAR).')
        print('Weight formula: w = 1 + (rho - 1) * |M - median(M)|')
        print('More extreme values have higher observation probability.\n')
    sys.stdout.flush()
...
def run_single_experiment(M_true, k, alpha, prop_obs, max_test_queries, max_calib_queries,
                          r, gamma_n=0, gamma_m=0, mu=1, mnar_weights=None, random_state=0):
    res = pd.DataFrame({})
    ...
    # Use MNAR weights if provided, otherwise uniform (MAR)
    mask_obs, mask_test = sampler.sample_submask(sub_size=prop_obs, w=mnar_weights, random_state=random_state)
...
for i in tqdm(range(1, repetition+1), desc="Repetitions", leave=True, position=0):
    ...
    for k in tqdm(k_list, desc="k", leave=True, position=0):
        ...
        res = run_single_experiment(M_true, k, alpha, prop_obs, max_test_queries, max_calib_queries,
                            r, gamma_n=gamma_n, gamma_m=gamma_m, mu=mu,
                            mnar_weights=mnar_weights, random_state=random_state)
